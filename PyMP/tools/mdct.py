...
def mdct(x , L):
    ...
    post_twidVec = array([cmath.exp((float(n) + 0.5) * -1j*pi*(L/2 +1)/L) for n in range(L/2)]) ;
...
def imdct(y , L):
    ...
    #Test length
    if mod(N, K)!= 0:
        print ["Input length " ,N , " must be a multiple of half window size : " , K]
        raise ValueError("The length was not a multipe of half the window size")
    ...
    # twidlle coefficients
    pre_twidVec = array([cmath.exp(1j*2*float(n)*pi*(float(float(L/4+0.5))/float(L))) for n in range(L)]);
    post_twidVec = array([cmath.exp(0.5*1j*2*pi*(float(n)+(L/4+0.5))/L) for n in range(L)]) ;

...
class DicoTest(unittest.TestCase):
    def runTest(self):
        ...
        # test dictionary class
        pyDico = mdct_dico.Dico([2**l for l in range(7, 15, 1)] ,debugLevel=3)
...
class Signaltest(unittest.TestCase):
    def runTest(self):
        ...
        pySig = signals.InitFromFile(audioFilePath +"ClocheB.wav" , True , True)
...
#
        ...
        L = 4*16384
        longSignal = signals.LongSignal(audioFilePath +"Bach_prelude_40s.wav", L)
        # suppose we want to retrieve the middle of the signals , namely from frame 5  to 12
        startSeg = 2;
        segNumber = 3;
        shortSignal = longSignal.getSubSignal(startSeg, segNumber, False)

        # witness signals
        witSignal = signals.InitFromFile(audioFilePath +"Bach_prelude_40s.wav", True, False)
...
#        plt.show()
        ...
        longSignal = signals.LongSignal(audioFilePath +"Bach_prelude_40s.wav", L, True, 0.5)
        # suppose we want to retrieve the middle of the signals , namely from frame 5  to 12
        startSeg = 2;
        segNumber = 3;
        shortSignal = longSignal.getSubSignal(startSeg, segNumber, False)
...
class BlockTest(unittest.TestCase):
    def runTest(self):
        ...
        block = mdct_block.Block(1024 , pySigOriginal , debugLevel=3 , useC=True);
...
#        plt.show()
        ...
        pySignal_oneAtom = signals.Signal(pyApprox_oneAtom.synthesize(0).dataVec, pySigOriginal.samplingFrequency, False)
...
#        del approximant
        ...
        # test two atoms
#        pyAtom2 = Atom.Atom(16384 , 1, 6*8192-1-4096 , 128 , 8000 , Atom.transformType.MDCT , -0.42)
...
#        self.assertTrue( isinstance(approximant , Approx.Approx));
        ...
        pyDico2 = mdct_dico.Dico([128 , 256 , 512 , 1024 , 2048 , 4096, 8192 , 16384]  ,parallel=False)
...
#        self.writeXmlTest()
        ...
        pySigOriginal = signals.InitFromFile(audioFilePath +"ClocheB.wav" , True);
...
#        del approxSignal
        ...
        pyApprox.addAtom(mdct_atom.Atom(8192 , 1 , 4096 - 2048 , 32 , 44100 , -0.24));

...
# print "Escaping Iteration ",iterationNumber,": mean " , mean , " std " , std
        ...
        for sigIdx in range(len(residualSignalList)):
            ...
            if not escapeThisAtom:
                ...
                try:
                    residualSignalList[
                        sigIdx].subtract(bestAtomList[sigIdx], debug)
                except ValueError:
                    if silentFail:
                        continue
                    else:
...
# currentApproxList[sigIdx].recomposedSignal.dataVec +
# escapeApproxList[sigIdx].recomposedSignal.dataVec;
# estimSources[sigIdx+1, :,0] =
# escapeApproxList[sigIdx].recomposedSignal.dataVec;
...
#        for sigIdx in range(len(residualSignalList)):
# residualSignalList[sigIdx].dataVec +=
# escapeApproxList[sigIdx].recomposedSignal.dataVec;
#        if doEval:
# return meanApprox, currentApproxList, resEnergyList , residualSignalList ,
# escapeApproxList , criterions , SDRs, SIRs , SARs

...
print (quantizedApprox.atom_number,  quantizedApproxLO.atom_number , quantizedApproxRSS.atom_number)
...
SNR, bitrate, quantizedApprox = mp_coder.simple_mdct_encoding(mpApprox, 64000, Q=16)
print (SNR, bitrate)

lompApprox , lompDecay = mp.mp(myPympSignal, pyLODico, 50, 20000,padSignal=False)  
SNRlo, bitratelo, quantizedApproxLO = mp_coder.simple_mdct_encoding(lompApprox, 64000, Q=16, TsPenalty=True)
print (SNRlo, bitratelo)

rssApprox , rssDecay = mp.mp(myPympSignal, pyRSSDico, 50, 20000,padSignal=False)  
SNRrss, bitraterss, quantizedApproxRSS = mp_coder.simple_mdct_encoding(rssApprox, 64000, Q=16)
print (SNRrss, bitraterss)
